...
thead.forEach((header, index)=>{
    header.addEventListener("click", ()=>{
        ...
        rows.sort((rowA, rowB)=>{
            ...
            if (index === 3) switch(order){
                case "ASC":
                    return Number(cellA) - Number(cellB);
                case "DESC":
                    return Number(cellB) - Number(cellB);
            }
            else if (index === 4) switch(order){
                case "ASC":
                    return Number(cellA.replace(/[$,]/g, "")) - Number(cellB.replace(/[$,]/g, ""));
                case "DESC":
                    return Number(cellB.replace(/[$,]/g, "")) - Number(cellA.replace(/[$,]/g, ""));
            }
            else switch(order){
                case "ASC":
...
function generateElement(position, object) {
    ...
    let fieldPlace = "";
    if (typeof field === "string") fieldPlace = document.createElement(field);
    else {
...
cells.forEach((cell)=>{
    cell.addEventListener("dblclick", ()=>{
        ...
        input.addEventListener("keydown", (e)=>{
            if (e.key === "Enter") cell.textContent = input.value || content;
        });
    });
});
